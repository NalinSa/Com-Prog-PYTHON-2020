...
def top_k_min_temp_stations(stations, K):
    lis =[]
    for key in stations:
        lis.append([stations[key]['temp'],key])
    lis.sort()
    lis = lis[:K:]
    sta = []
    for tem,stat in lis:
...
def peak_stations(stations):
    ...
    lis = [0]*4
    for key in stations:
        if stations[key]['lat'] < minlat:
            lis[0] = key
            minlat = stations[key]['lat']
        if stations[key]['lat'] > maxlat:
            lis[1] = key
            maxlat = stations[key]['lat']
        if stations[key]['long'] < minlong:
            lis[2] = key
            minlong = stations[key]['long']
        if stations[key]['long'] > maxlong:
            lis[3] = key
            maxlong = stations[key]['long']
    return lis

def k_nearby_stations(stations, main_station, K):
    lat2 = stations[main_station]['lat']
    long2 = stations[main_station]['long']
    lis = []
    for key in stations:
        lat1 = stations[key]['lat']
        long1 = stations[key]['long']
