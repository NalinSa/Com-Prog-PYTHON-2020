...
def update_v(x1, y1, r1, v1x, v1y, 
    ...
    s1 = math.atan2(v1y,v1x)
    s2 = math.atan2(v2y,v2x)
    p = math.atan2((y1-y2),(x1-x2))
    m1 = r1**2
    m2 = r2**2
    v1 = math.sqrt(v1y**2+v1x**2)
    v2 = math.sqrt(v2y**2+v2x**2)
    v1_x = (v1*math.cos(s1-p)*(m1-m2)+2*m2*v2*math.cos(s2-p))/(m1+m2)*math.cos(p)+v1*math.sin(s1-p)*math.cos(p+math.pi/2)
    v1_y = (v1*math.cos(s1-p)*(m1-m2)+2*m2*v2*math.cos(s2-p))/(m1+m2)*math.sin(p)+v1*math.sin(s1-p)*math.sin(p+math.pi/2)
    v2_x = (v2*math.cos(s2-p)*(m2-m1)+2*m1*v1*math.cos(s1-p))/(m1+m2)*math.cos(p)+v2*math.sin(s2-p)*math.cos(p+math.pi/2)
    v2_y = (v2*math.cos(s2-p)*(m2-m1)+2*m1*v1*math.cos(s1-p))/(m1+m2)*math.sin(p)+v2*math.sin(s2-p)*math.sin(p+math.pi/2)
